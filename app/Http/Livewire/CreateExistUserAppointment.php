...
class CreateExistUserAppointment extends Component
{
    ...
    public function OnlineDoctorData(){
        if($this->user){
            $this->doct = Doctor::where('has_zoom',1)->where('user_id','!=',$this->user)->get();
        }else{
    ...
    public function OfflineDoctorData(){
        if($this->user){
            $this->doct = Doctor::where('user_id','!=',$this->user)->get();
        }else {
    ...
    public function store(Request $request){
        ...
        if($validated) {
                ...
                $user_booking->user_id = $this->user;
                ...
                Session::flash('success','Create New Appointment Successfully for '.$user->full_name);
                $this->dispatchBrowserEvent('close-exist-user-modal');
                $this->reset(['user','object_id','is_online','user_object_booking','slotTime']);
                return redirect()->route('admin.date.index');
        }
    }
    ...
    {
        return view('livewire.create-exist-user-appointment');
    }
}

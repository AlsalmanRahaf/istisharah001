...
exports.consultant = (socket) => {
    ...
    socket.on('referredConsultations', async (data) => {
        console.log("tesssssssssssssssssssssssssssssssssssssssssssssssssss");
        ...
        await Room.create({
            ...
            await User.findOne({where: {type: RCType}}).then( async function (consaltent) {
                ...
                if(consaltent !== null){
                    ...
                    await User.findOne({where: {id:data.user_id}}).then( async function (user){
                        ...
                        userChatNamespace.to(user.id).emit("dialog-spesialist_chat",{kind:RCType});
                        userChatNamespace.to(user.id).emit("show_spesialist_chat",{
                            room_id:room_id_str,
                            user_name:user.full_name
                        })
                    });
                    ...
                    consultantNamespace.to(data.consultant_id).emit("Error_spesialist",{type:SpesialistType})
                    userChatNamespace.to(data.user_id).emit("Error_spesialist",{type:SpesialistType})
                }
                //
            });
        })
                ...
                await User.findOne({where: {id: ReConsultation.consultant_id}}).then(async function (user) {
                    ...
                    switch (data.status) {
                        ...
                        case 2 :
                            ...
                            userChatNamespace.to(data.user_id).emit("check_spesialist_conversation_status", {
                                status: data.status,
                                name: user.full_name
                            });
                            break;
                    ...
                    consultantNamespace.to(socket.request.user.id).emit("get_change_status", {"status": "success"})
                });
            });
        });
